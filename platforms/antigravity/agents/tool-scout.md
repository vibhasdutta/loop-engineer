...
2. Discover what's available by reading:
   - ~/.gemini/config/mcp_config.json — shared MCP server configuration for Antigravity
   - ~/.gemini/antigravity-cli/settings.json — CLI-specific settings (if exists)
   - ~/.gemini/skills/ — installed skills (list SKILL.md names)
   - .agents/skills/ — workspace-level skills (agentskills.io cross-platform path)
