...
2. Discover what's available by reading:
   - .cursor/mcp.json (project MCP config) or ~/.cursor/mcp.json (global MCP config) — MCP servers configured
   - ~/.cursor/skills/ — installed skills (list SKILL.md names)
   - .cursor/ in the current project — any project-level settings or agents
